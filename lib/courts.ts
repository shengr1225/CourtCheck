...
export async function listCourts(): Promise<ApiCourt[]> {
    const data = await apiFetch("/api/courts");
    if (!data.ok || !Array.isArray(data.courts)) throw new Error("Failed to load courts");
    return data.courts;
}
...
export async function createCheckin(
    ...
    if (!data.ok || !data.checkin) throw new Error(data.error || "Check-in failed");
    return data.checkin;
}

/** List check-ins for a court (latest first). Returns [] if endpoint not available (e.g. 404). */
export async function listCheckins(courtId: string): Promise<ApiCheckinWithUser[]> {
    try {
